...
public class ViewUserLogin {
	...
	/** Alert dialog to display when a username or password error occurs. */
	protected static Alert alertUsernamePasswordError = new Alert(AlertType.INFORMATION);

	/** Text field to capture the user's username. */
	protected static TextField text_Username = new TextField();
	
	/** Password field to capture the user's password securely. */
	...
	/** The Scene populated by each invocation of this view. */
	...
	/**
	 * <p> Description: Displays the user login page. It establishes references to the GUI, 
	 * initializes the static aspects if not yet established, and populates the dynamic 
	 * aspects (like clearing the input fields) before showing the scene.</p>
	 @param ps the JavaFX Stage to be used for this GUI and its methods
	 */
	...
	/**
	 * <p> Description: This constructor is called when the application first starts. It must handle
	 * two cases: 1) when no one has been established and 2) when one or more users have been 
	 * established. </p>
	 * <p> If there are no users in the database, this means that the person starting the system must
	 * be an administrator, so a special GUI is provided to allow this Admin to set a username and
	 * password. </p>
	 * <p> If there is at least one user, then a different display is shown for existing users to login
	...
	private ViewUserLogin() {
		...
		theUserLoginScene.getStylesheets().add(getClass().getResource("/"
				+ "applicationMain/application.css").toExternalForm());
		
		DialogPane alertUsernameOrPasswordErrorPane = alertUsernamePasswordError.getDialogPane();
		alertUsernameOrPasswordErrorPane.getStylesheets().add(getClass().getResource("/applicationMain/application.css").toExternalForm());
		alertUsernameOrPasswordErrorPane.getStyleClass().add("alertStyle");
